...
async function run() {
    try {
        ...
        const titleRegex = '^.*\b( \+semver\: breaking | \+semver\: feature | \+semver\: fix | \+semver\: none )\b.*$';
        const matches = title.match(new RegExp(titleRegex));
        if (!matches) {
        ...
        //const packageVersionIncrementationText = inputs.titleTemplate.replace(tokenRegex, match(inputs.uppercaseTitle));
        ...
        //updateTitle = true;
        //if (updateTitle) {
        //    request.title = titlePrefix.concat(title, ' ', packageVersionIncrementationText);
